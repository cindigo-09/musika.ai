...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: "Hi there! I'm your music assistant. Looking for a specific vibe, want to check a song, or create a playlist?" }
  ]);
  ...
  const detectIntentAndFetchContext = async (userMsg) => {
    ...
    if (msgLower.startsWith('play ') || msgLower.includes('play me ')) {
      ...
      const songs = await supabaseQuery(`/rest/v1/songs?title=ilike.*${encodeURIComponent(titleGuess)}*&select=id,title,artist,genre,mood_tag,song_url&limit=1`);
    ...
    else if (msgLower.includes('what songs do i have') || msgLower.includes('list songs') || msgLower.includes('my songs') || msgLower.includes('available songs') || msgLower.includes('what songs are')) {
      const songs = await supabaseQuery(`/rest/v1/songs?select=title,artist,genre,mood_tag&limit=20`);
      if (songs) {
    ...
    else if (msgLower.includes('available') || msgLower.includes('is ') && msgLower.includes(' in ')) {
      // Rough extraction of title (can be improved by AI later if needed)
      let titleGuess = userMsg.replace(/is |available| in the library|\?|\'|\"/gi, '').trim();
      const songs = await supabaseQuery(`/rest/v1/songs?title=ilike.*${encodeURIComponent(titleGuess)}*&select=title,artist,genre,mood_tag&limit=5`);
      if (songs) {
    ...
    // 2. Playlist Creation
      ...
      const moodMatch = msgLower.match(/for (.*?)$/i) || msgLower.match(/called (.*?)$/i) || msgLower.match(/of (.*?)$/i);

      if (msgLower.includes('random')) {
        playlistName = 'Random AI Playlist';
        // Random ordering can be simulated or we just take the first 10
      } else if (moodMatch) {
        let mood = moodMatch[1].trim().replace(/playlist/gi, '').trim();
        if (mood) {
          playlistName = mood.charAt(0).toUpperCase() + mood.slice(1) + ' Playlist';
      ...
      if (songs && songs.length > 0) {
        ...
        if (playlistData && playlistData[0]) {
          ...
          await supabaseQuery('/rest/v1/playlist_songs', 'POST', playlistSongsPayload);
          window.dispatchEvent(new CustomEvent('playlistCreated'));
          const songListNames = songs.map(s => `${s.title} by ${s.artist}`).join(', ');
    ...
    // 3. Vibe / Genre / Mood Search (Fallback)
      ...
      let searchTag = '';
      const genreMatch = msgLower.match(/([a-z0-9\-]+)\s+genre/i) || msgLower.match(/genre\s+([a-z0-9\-]+)/i);

      if (genreMatch) {
        searchTag = genreMatch[1].trim();
      } else {
        // Fallback to finding a descriptive word
        const ignoreWords = ['songs', 'music', 'with', 'like', 'that', 'are', 'about', 'the', 'some', 'play', 'show', 'give', 'me', 'what'];
        let words = msgLower.split(' ').filter(w => w.length > 2 && !ignoreWords.includes(w));
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      let apiMessages = [
        ...
        { role: 'user', content: userMessage + (dbContext ? `\n\n${dbContext}` : '') }
      ];
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'assistant', content: "Sorry, I ran into an error connecting to the music database or AI provider." }]);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {isOpen && (
        <div className="chatbot-window shadow-lg d-flex flex-column">
          ...
          <div className="chatbot-suggestions d-flex gap-2 p-2 overflow-auto">
            <span className="badge chatbot-badge" onClick={() => handleSuggestionClick("🌙 Late night drive")}>🌙 Late night drive</span>
            <span className="badge chatbot-badge" onClick={() => handleSuggestionClick("💪 Workout songs")}>💪 Workout songs</span>
            <span className="badge chatbot-badge" onClick={() => handleSuggestionClick("➕ Create a playlist")}>➕ Create a playlist</span>
            <span className="badge chatbot-badge" onClick={() => handleSuggestionClick("🔍 Check a song")}>🔍 Check a song</span>
          </div>
          ...
          <div className="chatbot-messages flex-grow-1 p-3 overflow-auto">
            {messages.map((msg, idx) => (
              <div key={idx} className={`d-flex mb-3 ${msg.role === 'user' ? 'justify-content-end' : 'justify-content-start'}`}>
                <div className={`chatbot-bubble ${msg.role === 'user' ? 'user-bubble' : 'ai-bubble'}`}>
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={sendMessage} className="chatbot-input p-3 d-flex gap-2 border-top">
            <input
              type="text"
              ...
              disabled={isLoading}
            />
            <button type="submit" className="btn chatbot-send-btn d-flex align-items-center justify-content-center" disabled={isLoading || !inputValue.trim()}>
              <Send size={18} />
            </button>
          </form>
        </div>
